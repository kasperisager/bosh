...
Bosh is a minimal Unix shell that allows for basic command interpretation and execution. It supports process communication via piping (`|`) in addition to file I/O redirects (`>` and `<`) and backgrounding (`&`).
...
This will compile Bosh and place its binary in `/usr/local/bin/`.
...
-   __Execution__  
    The final step is execution of the parsed commands. For each command, the executor takes care of setting up the pipes and file I/O redirects after spawning a separate process that will be used for executing the program specified by the command.
